// engine.evaluate() ?? e depois program.byteSize(xxx) ??
interface IByteSize {
    fromBytes(bytes: number): IByteSize;
    fromKiloBytes(bytes: number): IByteSize;
...
ByteSize.fromBytes = function (bytes: number) {
    //console.log('from bytes', bytes);
    return new ByteSize(bytes);
}

ByteSize.fromKiloBytes = function (bytes: number) {
    //console.log('from k bytes', bytes);
    return new ByteSize(bytes * 1000);
}

ByteSize.prototype.toString = function(format?: number) {
    if (format === undefined) {
        format = 2;
    }
    var output = function(value: number, symbol: string) {
